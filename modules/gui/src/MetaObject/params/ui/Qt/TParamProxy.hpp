...
namespace UI {
    namespace qt {
        ...
        protected:
            void onParamUpdate(typename ParamTraits<T>::ConstStorageRef_t data, IParam* param, Context* ctx, OptionalTime_t ts, size_t fn, const std::shared_ptr<ICoordinateSystem>& cs, UpdateFlags fg) {
                (void)ctx;
                (void)ts;
                (void)fn;
                (void)cs;
            ...
            void onParamDelete(IParam const* param) {
                if (param == this->_param) this->_param = nullptr;
            }
...
#define MO_UI_QT_PARAMTERPROXY_METAParam(N)                                                                                        \
    template <class T>                                                                                                             \
    struct MetaParam<T, N, typename std::enable_if<!UI::qt::ParamProxy<T>::IS_DEFAULT>::type> : public MetaParam<T, N - 1, void> { \
    ...
    template <class T>                                                                                                             \
    UI::qt::Constructor<T> MetaParam<T, N, typename std::enable_if<!UI::qt::ParamProxy<T>::IS_DEFAULT>::type>::_Param_proxy_constructor;
