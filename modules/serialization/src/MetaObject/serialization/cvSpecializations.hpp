...
namespace cereal
{
    ...
    void serialize(AR& ar, cv::Rect_<T>& rect)
    {
        ar(make_nvp("x", rect.x),
           make_nvp("y", rect.y),
           make_nvp("width", rect.width),
           make_nvp("height", rect.height));
    }
...
template <typename T>
std::ostream& operator<<(std::ostream& out, cv::Point_<T>& pt)
{
    out << pt.x << ", " << pt.y;
    return out;
}
...
template <typename T>
std::istream& operator>>(std::istream& in, cv::Point_<T>& pt)
{
    char ch;
    in >> pt.x >> ch >> pt.y;
    return in;
...
template <typename T>
std::ostream& operator<<(std::ostream& out, cv::Point3_<T>& pt)
{
    out << pt.x << ", " << pt.y << ", " << pt.z;
    return out;
}
...
template <typename T>
std::istream& operator>>(std::istream& in, cv::Point3_<T>& pt)
{
    char ch;
...
template <typename T>
std::ostream& operator<<(std::ostream& out, cv::Rect_<T>& pt)
{
...
template <typename T>
std::istream& operator>>(std::istream& in, cv::Rect_<T>& pt)
{
    char ch;
