...
#define MO_KEYWORD_INPUT(name, type)                                                                                                  \
    ...
    struct name {                                                                                                                     \
        ...
        template <typename T>                                                                                                         \
        static constexpr bool AllowedType() { return std::is_same<boost::optional<type>, T>::value || std::is_same<Type, T>::value; } \
...
#define ASSERT_EQUALITY(V1, V2) static_assert(              \
    AssertEquality<static_cast<int>(V1),                    \
                   static_cast<int>(V2)>::not_equal_warning \
        == 0,                                               \
    #V1 " != " #V2);
...
GetKeyImpl(const T& arg, const Args&... args) {
    return std::is_same<typename T::TagType, Tag>::value ? const_cast<void*>(arg.get()) : const_cast<void*>(GetKeyImpl<Tag, Args...>(args...));
}
