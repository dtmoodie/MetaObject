...
namespace mo{
    namespace Buffer{
        ...
        template<class T> typename std::map<SequenceKey, typename Map<T>::InputStorage_t>::iterator  Map<T>::search(OptionalTime_t ts){
        ...
        template<class T> typename std::map<SequenceKey, typename Map<T>::InputStorage_t>::iterator Map<T>::search(size_t fn){
            if (_data_buffer.size() == 0)
                return _data_buffer.end();
