...
namespace mo
{
    bool loadPlugin(const std::string& str)
    {
        return mo::MetaObjectFactory::instance()->loadPlugin(str);
    }
    ...
    {
        boost::python::object plugins_module(boost::python::handle<>(boost::python::borrowed(PyImport_AddModule((module_name + ".plugins").c_str()))));
        boost::python::scope().attr("plugins") = plugins_module;
        // set the current scope to the new sub-module
        boost::python::scope plugins_scope = plugins_module;
        
        boost::python::class_<PluginInfo, boost::shared_ptr<PluginInfo>, boost::noncopyable>("PluginInfo", boost::python::no_init)
