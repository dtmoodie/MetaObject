...
#define _CRT_SECURE_NO_WARNINGS // "secure" CRT not available on all platforms  --add this at the top of all CPP files
                                // that give "function or variable may be unsafe" warnings
#endif
...
#define MO_ASSERT_OP(op, lhs, rhs)                                                                                     \
    if (!(lhs op rhs))                                                                                                 \
    mo::ThrowOnDestroy(__FUNCTION__, __FILE__, __LINE__).stream()                                                      \
...
#define MO_ASSERT(exp)                                                                                                 \
    if (!(exp))                                                                                                        \
    mo::ThrowOnDestroy(__FUNCTION__, __FILE__, __LINE__).stream() << "[" << #exp << "] FAILED! "

#define CHECK_OP(op, lhs, rhs, severity)                                                                               \
    if (lhs op rhs)                                                                                                    \
    MO_LOG(severity) << "[" << #lhs << " " << #op << " " << #rhs << "] - Failed (" << lhs << " " << #op << " " << rhs  \
                     << ")"
...
namespace mo
{
    ...
    void MO_EXPORTS collectCallstack(size_t skipLevels,
    ...
    struct MO_EXPORTS ThrowStream_trace : public ThrowStream
    {
        ~ThrowStream_trace();
        ThrowStream_trace&
        operator()(int error, const char* file, int line, const char* func, bool collect_callstack = true);
    };
