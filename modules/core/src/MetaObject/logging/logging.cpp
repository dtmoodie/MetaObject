...
void mo::InitLogging()
{
    ...
    boost::log::formatter consoleFmt = boost::log::expressions::format("%1%<%2%") % fmtSeverity // 3
...
#else // BUGBUG: we cannot compile the above on Cygwin GCC, so for now fake it using the mbs functions, which will only
// work for 7-bit ASCII strings
        ...
        for (size_t i = name.size(); i-- > 0;)
        {
            ...
            if (name[i] == ',')
            {
                ...
                if (!hitEnd && angleDepth == 0 && parenDepth == 1)
                    name.insert(
                        i + 1,
                        "  "); // except for top-level arguments, we separate them by 2 spaces for better readability
            }
            // function name
            if ((name[i] == '(' || name[i] == '<') && parenDepth == 0 && angleDepth == 0 &&
                (i == 0 || name[i - 1] != '>') && !hitEnd && !hitStart) // we hit the start of the argument list
            {
                hitEnd = true;
                name.insert(i, "  ");
            }
            else if ((name[i] == ' ' || name[i] == ':' || name[i] == '>') && hitEnd && !hitStart &&
                     i > 0) // we hit the start of the function name
            {
                if (name[i] != ' ')
                    name.insert(i + 1, " ");
...
void mo::collectCallstack(size_t skipLevels,
                          ...
                          const std::function<void(const std::string&)>& write)
{
    ...
    // RtlCaptureStackBackTrace() is a kernel API without default binding, we must manually determine its function
    // pointer.
    if (RtlCaptureStackBackTrace_func == nullptr)
        RtlCaptureStackBackTrace_func =
            (CaptureStackBackTraceType)(GetProcAddress(LoadLibrary("kernel32.dll"), "RtlCaptureStackBackTrace"));
    if (RtlCaptureStackBackTrace_func == nullptr) // failed somehow
        return write("Failed to generate CALL STACK. GetProcAddress(\"RtlCaptureStackBackTrace\") failed with error " +
                     utf8(FormatWin32Error(GetLastError())) + "\n");

    HANDLE process = GetCurrentProcess();
    if (!SymInitialize(process, nullptr, TRUE))
        return write("Failed to generate CALL STACK. SymInitialize() failed with error " +
                     utf8(FormatWin32Error(GetLastError())) + "\n");
    ...
    SYMBOL_INFO* symbolInfo = (SYMBOL_INFO*)calloc(sizeof(SYMBOL_INFO) + 256 * sizeof(char),
                                                   1); // this is a variable-length structure, can't use vector easily
...
{
    log_stream_ << "[" << file << ":" << line << " - " << function << "]";
}
...
{
    std::stringstream ss;
    MO_LOG(debug) << "Exception at" << mo::printCallstack(0, true, ss) << log_stream_.str();
...
mo::ThrowOnDestroy_trace::ThrowOnDestroy_trace(const char* function, const char* file, int line)
...
{
    std::stringstream ss;
    MO_LOG(trace) << "Exception at" << mo::printCallstack(0, true, ss) << log_stream_.str();
    if (!_currently_throwing_exception)
    {
        _currently_throwing_exception = true;
        throw mo::ExceptionWithCallStack<std::string>(log_stream_.str(), ss.str());
...
mo::ThrowOnDestroy_debug::ThrowOnDestroy_debug(const char* function, const char* file, int line)
    : ThrowOnDestroy(function, file, line)
{
}

mo::ThrowOnDestroy_info::ThrowOnDestroy_info(const char* function, const char* file, int line)
    : ThrowOnDestroy(function, file, line)
{
}
...
{
    std::stringstream ss;
    MO_LOG(info) << "Exception at" << mo::printCallstack(0, true, ss) << log_stream_.str();
    if (!_currently_throwing_exception)
    {
        _currently_throwing_exception = true;
        throw mo::ExceptionWithCallStack<std::string>(log_stream_.str(), ss.str());
...
mo::ThrowOnDestroy_warning::ThrowOnDestroy_warning(const char* function, const char* file, int line)
    : ThrowOnDestroy(function, file, line)
{
}
...
{
    std::stringstream ss;
    MO_LOG(warning) << "Exception at" << mo::printCallstack(0, true, ss) << log_stream_.str();
    if (!_currently_throwing_exception)
    {
        _currently_throwing_exception = true;
        throw mo::ExceptionWithCallStack<std::string>(log_stream_.str(), ss.str());
...
namespace mo
{
    ...
    {
        msg.str(std::string());
        msg << file << ":" << line << " " << error << " in function [" << func << "]";
