...
static std::string MakeFunctionNameStandOut(std::string origName) {
    try // guard against exception, since this is used for exception reporting
    {
        ...
        for (size_t i = name.size(); i-- > 0;) {
            ...
            // function name
            if ((name[i] == '(' || name[i] == '<') && parenDepth == 0 && angleDepth == 0 && (i == 0 || name[i - 1] != '>') && !hitEnd && !hitStart) // we hit the start of the argument list
            {
                hitEnd = true;
                name.insert(i, "  ");
...
ThrowStream& mo::ThrowStream::operator()(int error, const char* file, int line, const char* func, bool collect_callstack) {
    msg.str(std::string());
...
ThrowStream_trace& ThrowStream_trace::operator()(int error, const char* file, int line, const char* func, bool collect_callstack) {
    msg.str(std::string());
    msg << file << ":" << line << " " << error << " in function [" << func << "]";
    if (collect_callstack)
        callstack = printCallstack(1, true);
...
ThrowStream_debug& ThrowStream_debug::operator()(int error, const char* file, int line, const char* func, bool collect_callstack) {
    msg.str(std::string());
    msg << file << ":" << line << " " << error << " in function [" << func << "]";
    if (collect_callstack)
        callstack = printCallstack(1, true);
...
ThrowStream_info& ThrowStream_info::operator()(int error, const char* file, int line, const char* func, bool collect_callstack) {
    msg.str(std::string());
    msg << file << ":" << line << " " << error << " in function [" << func << "]";
    if (collect_callstack)
        callstack = printCallstack(1, true);
...
ThrowStream_warning& ThrowStream_warning::operator()(int error, const char* file, int line, const char* func, bool collect_callstack) {
    msg.str(std::string());
    msg << file << ":" << line << " " << error << " in function [" << func << "]";
    if (collect_callstack)
        callstack = printCallstack(1, true);
...
ThrowStream_error& ThrowStream_error::operator()(int error, const char* file, int line, const char* func, bool collect_callstack) {
    msg.str(std::string());
    msg << file << ":" << line << " " << error << " in function [" << func << "]";
    if (collect_callstack)
        callstack = printCallstack(1, true);
