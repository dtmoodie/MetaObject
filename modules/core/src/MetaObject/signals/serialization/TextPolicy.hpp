...
namespace mo
{
    ...
    {
      public:
        static void Deserialize(std::ostream& ss, std::tuple<T...>& args) { DeserializeImpl(ss, std::get<0>(args), 0); }
        static void Serialize(std::istream& ss, std::tuple<T...>& args) { SerializeImpl(ss, std::get<0>(args), 0); }
    };
    ...
    class TextSlotCaller<R(T...)> : public ISignalCaller
    {
      public:
        ...
        TextSlotCaller(TSlot<R(T...)>* slot) { _slot = slot; }
