...
void testGetSignal(const std::string& name, IMetaObject* obj)
{
    ...
    ASSERT_NE(signal, nullptr) << "Unable to retrieve signal with name '" << name << "' and signature [" << TypeInfo::create<SIG>().name() << "] from object of type " << obj->GetTypeName() << "Available signals:\n" << obj->getSignals();
...
TEST(signal, init_and_access)
{
    ...
    testGetSignal<void(IMetaObject*, mo::Header, IParam*)>("param_updated", obj.get());
