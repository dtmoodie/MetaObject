...
BOOST_AUTO_TEST_CASE(serialize_multi_by_policy_binary)
{
    ...
    }
    {
        std::ifstream ifs("test2.bin", std::ios::binary);
        auto new_obj1 =
            rcc::shared_ptr<serializable_object>(SerializerFactory::DeSerialize(ifs, SerializerFactory::Binary_e));
        auto new_obj2 =
            rcc::shared_ptr<serializable_object>(SerializerFactory::DeSerialize(ifs, SerializerFactory::Binary_e));
...
BOOST_AUTO_TEST_CASE(deserialize_text_path)
{
    ...
    auto deserialization_function =
        mo::SerializationFactory::instance()->getTextDeSerializationFunction(param.getTypeInfo());
...
BOOST_AUTO_TEST_CASE(serialize_text_vector)
{
    ...
    param.updatePtr(&data);
    auto serialization_function =
        mo::SerializationFactory::instance()->getTextSerializationFunction(param.getTypeInfo());
