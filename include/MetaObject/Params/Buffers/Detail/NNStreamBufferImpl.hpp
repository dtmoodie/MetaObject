...
template<class T>
typename std::map<SequenceKey, T>::iterator NNStreamBuffer<T>::Search(OptionalTime_t ts)
{
...
template<class T>
T* NNStreamBuffer<T>::GetDataPtr(OptionalTime_t ts, Context* ctx, size_t* fn)
{
...
template<class T>
T* NNStreamBuffer<T>::GetDataPtr(size_t fn_, Context* ctx_, OptionalTime_t* ts_)
{
	mo::Mutex_t::scoped_lock lock(IParam::mtx());
	auto itr = Search(fn_);
	if (itr != this->_data_buffer.end())
...
template<class T>
T NNStreamBuffer<T>::GetData(OptionalTime_t ts, Context* ctx, size_t* fn)
{
    mo::Mutex_t::scoped_lock lock(IParam::mtx());
	auto itr = Search(ts);
	if (itr != this->_data_buffer.end())
...
template<class T>
T NNStreamBuffer<T>::GetData(size_t fn, Context* ctx, OptionalTime_t* ts)
{
	mo::Mutex_t::scoped_lock lock(IParam::mtx());
	auto itr = Search(fn);
	if (itr != this->_data_buffer.end())
...
template<class T>
bool NNStreamBuffer<T>::GetData(T& value, OptionalTime_t ts, Context* ctx, size_t* fn)
{
	mo::Mutex_t::scoped_lock lock(IParam::mtx());
	auto itr = Search(ts);
	if (itr != this->_data_buffer.end())
...
template<class T>
bool NNStreamBuffer<T>::GetData(T& value, size_t fn, Context* ctx, OptionalTime_t* ts)
{
	mo::Mutex_t::scoped_lock lock(IParam::mtx());
	auto itr = Search(fn);
	if (itr != this->_data_buffer.end())
